...
def classify_data(data_set_train, data_set_test):
    # load a dataset
    ...
    # remove unprocessed STRING attributes: rm_key and year
    remove_str = Filter(classname='weka.filters.unsupervised.attribute.Remove',\
                                           options=['-R', '111,112'])
    remove_str.inputformat(train)
    no_strings = remove_str.filter(train)
    ...
    f = open('false_predictions_phaseII.txt','w')
    for index, inst in enumerate(test):
        ...
        if (inst.get_value(inst.class_index) != prediction):
            f.write(" Rm_key: " + str(inst.get_string_value(110)) +  
                    " Stmt_year: " + str(inst.get_string_value(111)) +
...
def main():	
    try:
        ...
        #classify data for phase II
        classify_data('housingdata_train.arff', 'housingdata_test.arff')
